...
__turbopack_context__.s({
    ...
    "mockApps": (()=>mockApps),
    "mockPaymentButtons": (()=>mockPaymentButtons),
    "mockRefunds": (()=>mockRefunds),
    "mockWithdrawals": (()=>mockWithdrawals)
});
...
const mockPaymentButtons = {
    getPaymentButtons: async (filters)=>{
        ...
        if (filters.status) {
            filteredButtons = filteredButtons.filter((button)=>button.status === filters.status);
        }
        if (filters.search) {
            const searchTerm = filters.search.toLowerCase();
            filteredButtons = filteredButtons.filter((button)=>button.name.toLowerCase().includes(searchTerm) || button.buttonToken.toLowerCase().includes(searchTerm));
        }
        // Apply pagination
...
const mockRefunds = {
    getRefunds: async (filters)=>{
        ...
        // Filtro por tipo
        if (filters.type) {
            filteredData = filteredData.filter((refund)=>refund.type === filters.type);
        }
        // Filtro por pesquisa (ID, transaction_id ou nome do cliente)
        if (filters.search) {
            const searchTerm = filters.search.toLowerCase();
            filteredData = filteredData.filter((refund)=>refund.id.toLowerCase().includes(searchTerm) || refund.transaction_id.toLowerCase().includes(searchTerm) || refund.customer.name.toLowerCase().includes(searchTerm));
        }
        // Paginação
        ...
        const totalAmount = filteredData.reduce((sum, refund)=>sum + refund.amount, 0);
        return {
    ...
    updateRefundStatus: async (id, status, approvedBy)=>{
        ...
        if (refundIndex !== -1) {
            const updatedRefund = {
                ...
                approved_at: status === 'completed' ? new Date().toISOString() : undefined,
                processed_at: [
                    'completed',
                    'failed'
                ].includes(status) ? new Date().toISOString() : refundsData[refundIndex].processed_at
            };
...
const fakeProducts = {
    ...
    initialize () {
        ...
        function generateRandomProductData(id) {
            ...
            const name = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$faker$2d$js$2f$faker$2f$dist$2f$chunk$2d$TVFJBHBT$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$export__a__as__faker$3e$__["faker"].commerce.productName();
            return {
                id,
                name,
                ...
                quantidade: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$faker$2d$js$2f$faker$2f$dist$2f$chunk$2d$TVFJBHBT$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__$3c$export__a__as__faker$3e$__["faker"].number.int({
                    min: 1,
                    max: 100
                }),
...
const mockWithdrawals = {
    getWithdrawals: async (filters)=>{
        ...
        if (filters.search) {
            ...
            filtered = filtered.filter((w)=>w.id.toLowerCase().includes(search) || w.destination.toLowerCase().includes(search) || w.destination_name && w.destination_name.toLowerCase().includes(search) || w.reference && w.reference.toLowerCase().includes(search));
        }
        // Paginação
...
const mockApps = {
    getApps: async (filters)=>{
        ...
        if (filters.search) {
            filteredData = filteredData.filter((app)=>app.name.toLowerCase().includes(filters.search.toLowerCase()) || app.description?.toLowerCase().includes(filters.search.toLowerCase()));
        }
        // Paginação correta
        const page = filters.page || 1;
        const limit = filters.limit || 10;
...
;
const PaymentMethodsAreaChart = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$server$2d$dom$2d$turbopack$2d$server$2d$edge$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerClientReference"])(function() {
    throw new Error("Attempted to call PaymentMethodsAreaChart() from the server but PaymentMethodsAreaChart is on the client. It's not possible to invoke a client function from the server, it can only be rendered as a Component or passed to props of a Client Component.");
}, "[project]/src/features/overview/components/area-graph.tsx <module evaluation>", "PaymentMethodsAreaChart");
}}),
...
const PaymentMethodsAreaChart = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$server$2d$dom$2d$turbopack$2d$server$2d$edge$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerClientReference"])(function() {
    throw new Error("Attempted to call PaymentMethodsAreaChart() from the server but PaymentMethodsAreaChart is on the client. It's not possible to invoke a client function from the server, it can only be rendered as a Component or passed to props of a Client Component.");
}, "[project]/src/features/overview/components/area-graph.tsx", "PaymentMethodsAreaChart");
}}),
